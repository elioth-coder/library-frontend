<script>
  ...
  import CampusForm from "./CampusForm.svelte";
  import FormContainerModal from "../FormContainerModal.svelte";

  let table_name = "campus";
  let columns = ["name", "location"];
  let service = new CampusService();
  let modalSize = "xs";
...
>
  <CampusForm {processing} item={selectedItem} on:submit={submitItem} />
</FormContainerModal>
