<script>
  ...
  const getMembers = async () => {
    let members = await memberService.getBy("status", "Rejected");
...
<Layout>
  ...
  >
    <Breadcrumb {crumbs} />
    <div class="px-3">
      <Heading tag="h4" class="text-center">Library Members (Rejected)</Heading>
      <TableSearch
      ...
      >
        <TableHead>
          ...
          <TableHeadCell>Reason for Rejection</TableHeadCell>
        ...
        <TableBody tableBodyClass="divide-y">
          ...
          {:else}
            <TableBodyRow>
              <TableBodyCell colspan={6} class="text-center">No members found.</TableBodyCell>
            </TableBodyRow>
          {/each}
        </TableBody>
      </TableSearch>
